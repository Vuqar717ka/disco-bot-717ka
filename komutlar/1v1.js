...
exports.run = (client, message, args) => {
    if (!message.guild) {
      return message.channel.send(new Discord.RichEmbed().setColor('RANDOM').setTitle('Eval;').setDescription(message.author.username + ', bu komutu direkt mesajda kullanamazsın.').setFooter('DailyBot', client.user.avatarURL).setTimestamp()); }
    let user = message.mentions.users.first();
    if (message.mentions.users.size < 2) return message.channel.send(new Discord.RichEmbed().setColor('RANDOM').setTitle('1v1;').setDescription(message.author.tag + ', kullanım: p! 1vs1 <@kullanıcı> <@kullanıcı> .').setFooter('DailyBot', client.user.avatarURL).setTimestamp());
...
exports.help = {
  ...
  usage: 'savaş <@kullanıcı> <@kullanıcı>'
};